...
async function loadOrganizerEvents(organizerIndex, eventsList, organizerName) {
    try {
        ...
        console.log('All available events:', allEvents.map(e => ({ title: e.title, organizer: e.organizer, createdBy: e.createdBy })));
        ...
        const organizerEvents = allEvents.filter(e => {
            ...
            console.log(`Checking event "${e.title}": "${eventOrganizer}" === "${currentOrganizerLower}" ? ${matches}`);
            return matches;
        });
        ...
        if (organizerEvents.length === 0) {
            ...
            eventsList.innerHTML = organizerEvents.map(event => `
                <div style="
                    padding: 0.75rem;
                ...
                ">
                    <p style="font-weight: 500; color: var(--gray-900); font-size: 0.9rem; margin-bottom: 0.25rem;">
                        ${event.title || event.name || 'Untitled Event'}
                    </p>
                    <div style="display: flex; gap: 1rem; font-size: 0.8rem; color: var(--gray-600);">
                        <span>${event.date || 'N/A'}</span>
                        <span>${event.location || 'N/A'}</span>
                        <span style="color: var(--primary-blue); font-weight: 500;">${formatNumber(event.registrations || 0)} registrations</span>
                    </div>
                </div>
            `).join('');
        }
...
export async function renderOrganizersReport(container) {
            ...
            <div class="bg-gray-50" style="min-height: 100vh; padding: 2rem; display: flex; align-items: center; justify-content: center;">
                <div class="card" style="max-width: 500px;">
                    <div class="card-content" style="text-align: center;">
                        ...
                        <button class="btn btn-primary mt-4" onclick="navigateTo('admin-panel')">
                            Back to Admin Panel
                        </button>
                    </div>
                </div>
    ...
    const registeredOrganizers = Object.entries(users)
        .filter(([username, userData]) => userData.role === "organizer")
    ...
    const organizersWithCounts = registeredOrganizers.map(org => {
        const eventCount = allEvents.filter(event => {
            const eventOrganizer = (event.organizer || event.createdBy || event.creator || '').toString().toLowerCase().trim();
            const currentOrganizerLower = org.organizerName.toLowerCase().trim();
            return eventOrganizer === currentOrganizerLower;
        }).length;

        console.log(`Organizer: ${org.organizerName}, Event Count: ${eventCount}, Looking for: ${org.organizerName.toLowerCase().trim()}`);
        console.log(`Events matching this organizer:`, allEvents.filter(event => {
            const eventOrganizer = (event.organizer || event.createdBy || event.creator || '').toString().toLowerCase().trim();
            const currentOrganizerLower = org.organizerName.toLowerCase().trim();
            return eventOrganizer === currentOrganizerLower;
        }));
